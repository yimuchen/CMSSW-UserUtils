...
template <> void EDNtupleProducer::AddValue<float>(const std::string &name) {
  _float_map[name] = std::make_unique<float>();
  tree->Branch(name.c_str(), _float_map[name].get());
}

template <> void EDNtupleProducer::AddValue<int>(const std::string &name) {
  _int_map[name] = std::make_unique<int>();
  tree->Branch(name.c_str(), _int_map[name].get());
}

template <> void EDNtupleProducer::AddValue<bool>(const std::string &name) {
  _bool_map[name] = std::make_unique<bool>();
  tree->Branch(name.c_str(), _bool_map[name].get());
}

template <>
void EDNtupleProducer::SetValue<>(const std::string &name, const double x) {
  *_float_map[name] = x;
}

template <>
void EDNtupleProducer::SetValue<>(const std::string &name, const float x) {
  *_float_map[name] = x;
}

template <>
void EDNtupleProducer::SetValue<>(const std::string &name, const int x) {
  *_int_map[name] = x;
}

template <>
void EDNtupleProducer::SetValue<>(const std::string &name, const bool x) {
  *_bool_map[name] = x;
}

void EDNtupleProducer::analyze(const edm::Event &i, const edm::EventSetup &j) {
  clear_collections();
  if (np_analyze(i, j)) {
    tree->Fill();
  }
...
void EDNtupleProducer::clear_collections() {
