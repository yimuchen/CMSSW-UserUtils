#ifndef USERUTILS_EDMALIAS_EDNTUPLERPRODUCER
#define USERUTILS_EDMALIAS_EDNTUPLERPRODUCER
...
protected:
  ...
  template <typename> void AddCollection(const std::string &);
  template <typename T> std::vector<T> &Col(const std::string &);

  template <typename> void AddValue(const std::string &);
  template <typename T> void SetValue(const std::string &, const T);
